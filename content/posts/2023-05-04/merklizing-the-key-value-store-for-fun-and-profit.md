...
You can merklize a database _index_, and find that it unlocks a crazy new set of peer-to-peer primitives. Overall it seems like there's an emerging vision of a lower-level type of decentralization, where the data structures themselves sync directly with each other. The modern computer has hundreds of databases of various kinds for various apps at all times - what if sync and replication logic didn't have to get mediated by the application layer at all? What if you could make a social app without writing a single line of networking code at all?
